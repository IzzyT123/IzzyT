...
And once you see that cost clearly, you start to notice how much of an organisation exists to manage it. Layers of management are a way to route and filter information so that not everyone has to talk to everyone. Departments and functions draw a boundary around a manageable number of lines. Even the meeting is a scheduling hack for getting the right people's information into one place at one time. A surprising amount of what we call structure is really a workaround for communication being expensive, slow, and lossy.
...
And this reaches further than where we sit. If the cost of moving information is what shaped the org chart, then as that cost collapses the org chart itself is in play. The structure I described earlier, the management layers and the departments and the standing meetings, was largely built to move information around when moving it was expensive. Take the expense away and the parts that exist only to shuttle information, rather than to judge or decide, are the parts I would expect to thin out. Hierarchy does not disappear, because someone still has to decide and be accountable. But the real question stops being how many days we spend in the office and becomes what shape an organisation even wants once coordination is no longer the expensive part.
...
The habit I have found compounds more than any other is capturing meetings as transcripts. Once you have a transcript, and once any confidential details have been stripped out or made generic by hand, it can be dropped into a chatbot, a coding agent, a co-working agent, whatever fits. The AI strips out what is irrelevant and understands the full context of the conversation, even the parts that were not said cleanly. Nobody has to comb through with a fine-tooth comb to pull out the important bits, and nobody even has to have fully understood the meeting in the moment. I have noticed the model is often better than my own ear at catching what someone actually meant, as opposed to what I assumed they meant. Maybe I am just a bad listener. Either way, once a transcript is safe to use, the transfer of information becomes so fast that it speeds up everything downstream. The manual cleaning step is the catch, which is the next problem.
...
So what are the skills people will actually need? My short list is learning as a habit, adaptability, and curiosity. The shelf life of any specific piece of knowledge is heading towards zero, so resilience and a willingness to keep rebuilding your own workflow matter more than any individual skill. Curiosity is what keeps you experimenting instead of waiting to be told. And organisations have to hold up their end: build that mindset, agree the social contract, provide the access, opportunities, and tools for people to learn, and give people real ownership of what they create. If someone is effectively automating part of their own role, the honest questions are how they are rewarded for it, where they move next, and what the new role actually is.
...
I understand we build humanoids because people accept them, and that acceptance has real value. But in the long run, the company that builds the robots that get the tasks done with a hundred times less energy is the one that wins. This is really a game theory problem, and we are playing it from the wrong end. Instead of starting from what we assume evolution has already optimised, the human body, we should define the desired result and work backwards to the equilibrium, the way you solve for a Nash equilibrium by reasoning back from the outcome. The human body is the wrong anchor anyway, because the tasks we actually want done have evolved far faster than the body has, so the body was never optimised for most of them in the first place.
